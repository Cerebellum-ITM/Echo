...
| 47 | modstate                    | `modstate [--all] [--json]` dumps `ir_module_module` for the active DB — installed-only by default, `--all` for every state. Human mode prints an aligned `name \| state \| version` table; `--json` emits a clean JSON array to stdout only (null version → `null`), logs/errors to stderr. Headless, one-shot, `-C`-aware; reuses the DB plumbing, no manifest comparison (unlike `modinfo`) | 05, 09, 42 |
